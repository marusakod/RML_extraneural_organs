...
# get mouse hub genes
mouse_hubs <- read.csv('output/wgcna/muscle_b1_b1_hub_stats.csv')
...
orange_mouse = mouse_hubs %>% filter(module == 'orange') %>%
  filter(is.hub_b1_MM == TRUE,  is.hub_b2_MM == TRUE) %>%
  pull(gene_symbol) %>%
  mouse2human() %>%
  pull(humanGene)

darkgreen_mouse = mouse_hubs %>% filter(module == 'darkgreen') %>%
  filter(is.hub_b1_MM == TRUE, is.hub_b2_MM == TRUE) %>%
  pull(gene_symbol) %>%
  mouse2human() %>%
  pull(humanGene)
...
annotated_genes <- cjd_deseq2_res_for_volcano %>%
  filter(gene_symbol %in% c(orange_mouse, darkgreen_mouse)) %>%
...
annotated_genes_labels <- annotated_genes %>% filter(gene_type_binary == 'Significant')
 ...
 theme(legend.text = element_text(size = 8),
  ...
  geom_point(data = nonsignificant_anno,
     ...
     ggrepel::geom_text_repel(data = annotated_genes_labels,
                    aes(label = gene_symbol, size = gene_type_binary),
                    max.overlaps = Inf,
                    colour = "#220303",
                    fontface = "italic", 
...
```{r export_plots}
ggsave(CJD_pca_plot_file, CJD_pca.plo)
ggsave( CJD_volcano_file, volcano_plot,  width = 3.5, height = 3.7)
ggsave(glul_norm_counts_file, GLUL_norm_counts_plot, width = 1.45, height = 2 )
