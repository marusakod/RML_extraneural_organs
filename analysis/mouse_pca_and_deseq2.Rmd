...
```{r make_no_outlier_counts}
all_samples_meta_w_qc_no_outs <- mapply(x = all_samples_meta_w_qc,
                                        y = all_outliers, 
                                        FUN = function(x, y){
                                          x %>% filter(!SampleID %in% y)
                                        }, SIMPLIFY = FALSE)
# get counts
all_FC_counts_list_no_outs <- mapply(organ = c('blood', 'muscle_batch1', 'spleen'),
                             meta = all_samples_meta_w_qc_no_outs ,
                             FUN= get_feature_counts,
                             MoreArgs= list(counts_dir = Fcounts_main_dir,
                                            file_prefix = 'no_outliers_', 
                             overwrite = FALSE),
                             SIMPLIFY = FALSE) %>%
  setNames(organs)
...
# do pca without outliers

pcas_no_outs <- lapply(all_FC_counts_list_no_outs, FUN = do_pca)
pcas_dfs_no_outs <- mapply(all_samples_meta_w_qc_no_outs, pcas,FUN = get_pca_meta_df, SIMPLIFY = FALSE) %>%
  lapply(., function(x){
   x <- x %>% mutate(Treatment_wpi = paste(Treatment, gsub(' ', '_', wpi), sep = '.')) %>%
           mutate(Treatment_stage = paste(Treatment, stage, sep = '_'))
...
pca_plots_no_outs <- vector("list", length = 3) %>% setNames(organs)
for(i in organs){
  
  pca_plots_no_outs[[i]] <- make_pca_plot2(pcas_dfs_no_outs[[i]], 
                          sample_label = FALSE, 
                          proportional_axes = TRUE) 
}
...
stage_pca_plots <- lapply(pcas_dfs_stage, make_stage_pca, proportional_axes = TRUE, sample_label = TRUE)
