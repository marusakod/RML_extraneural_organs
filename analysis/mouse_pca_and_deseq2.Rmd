...
all_samples_meta_w_qc <- mapply(meta = all_samples_info_split, 
                                ...
                                MoreArgs = list(out_dir = out_dir, 
                                                ...
                                                overwrite = FALSE), 
                                SIMPLIFY = FALSE) %>%
  # add percent protein coding
  lapply(., function(x){
    x$pct_pc <- x[, "Protein-coding"]/x[, "lib_size"]
    x
  })

# for each organs get bottom outliers based on library size, feature number and % of protein coding genes


outliers_lib_size <- lapply(all_samples_meta_w_qc, function(x) detect_bottom_outliers(x, 'lib_size'))
outliers_feature_count <- lapply(all_samples_meta_w_qc, function(x) detect_bottom_outliers(x, 'feature_counts'))
outliers_pc_pct <- lapply(all_samples_meta_w_qc, function(x) detect_bottom_outliers(x, 'pct_pc'))

all_outliers <- lapply(organs, function(x){
  unique(c(outliers_lib_size[[x]], outliers_feature_count[[x]], outliers_pc_pct[[x]]))
})
...
pca_plots_3d <- lapply(pca_dfs, make_3d_pca_plot)
...
```{r make_stage_pca_plots}

all_samples_info_stage_organ_split <- all_samples_info %>%
  # filter(!SampleID %in% unlist(all_outliers)) %>%
  group_by(Region, stage) %>%
   group_split() %>%
   setNames(lapply(., function(x) paste(unique(x$stage), unique(x$Region), sep = '_'))) %>%
  # add rownames
  lapply(., function(x){
    x <-as.data.frame(x)
    rownames(x) <- x$SampleID
...
all_counts_stage_organ_split <- mapply(meta = all_samples_info_stage_organ_split, 
                                    organ = str_after_first(names(all_samples_info_stage_organ_split), pattern = '_'),
                                   FUN = extract_group_counts, 
                                   SIMPLIFY = FALSE)

stage_pcas <- lapply(all_counts_stage_organ_split, FUN = do_pca)
pcas_dfs_stage <- mapply(all_samples_info_stage_organ_split, stage_pcas,FUN = get_pca_meta_df, SIMPLIFY = FALSE) 
...
do.call('grid.arrange', c(stage_pca_plots, ncol = 3, nrow = 3))
...
all_deseqs_stage_organ_split <- vector("list", length  = length(all_counts_stage_organ_split))

for(i in 1:length(all_deseqs_stage_organ_split)){
  organ <- str_after_first(names(all_samples_info_stage_organ_split)[i], pattern = '_')
  counts <- all_counts_stage_organ_split[[i]]
  meta <- all_samples_info_stage_organ_split[[i]]

 all_deseqs_stage_organ_split[[i]] <- run_deseq2(counts = counts, 
                                                 meta = meta, 
                                                 ctrl_level = 'NBH', 
                                                 main_var = 'Treatment', 
                                                 formula = ~ Treatment)
  
  print(paste0('Finished for ', names(all_samples_info_stage_organ_split)[i]))
}


# get nice outputs

all_deseq2_stage_organ_split_nice <- mcmapply(dds = all_deseqs_stage_organ_split,
                                  fname_prefix = names(all_counts_stage_organ_split), 
                                  FUN = get_nice_df_from_dds, 
                                  MoreArgs = list(
                                  feature_meta = mouse_feature_meta, 
                                  meta_vars_to_add = c('wpi', 'Region', 'stage'), 
                                  out_dir = out_dir, overwrite = TRUE), SIMPLIFY = FALSE, mc.cores = 4) %>%
  setNames(names(all_counts_stage_organ_split))


all_common_genes <- organs %>%
  lapply(., function(x){
  all_deseq2_stage_organ_split_nice[grepl(x, names(all_deseq2_stage_organ_split_nice))] %>%
      common_genes()
  }) %>%
  setNames(organs)
...
all_deseq2_stage_organ_merged <- organs %>%
  lapply(., function(x){
  m <- bind_rows(all_deseq2_stage_organ_split_nice[grepl(x, names(all_deseq2_stage_organ_split_nice))]) %>%
...
# prepare metadata
all_samples_info_tp_organ_split <- all_samples_info %>%
  # remove outliers
  # filter(!SampleID %in% unlist(all_outliers)) %>% 
  group_by(Region, wpi) %>% 
  group_split() %>%
...
# find overlapping DEGs in early blood timepoints
blood_early_overlap <- lapply(all_deseq2_tp_organ_split_nice[c(1:2)], 
                              FUN = function(x){
                                #extract DEGS
                                x %>% filter(gene_type != 'Nonsignificant') %>%
                                  pull(ensembl_gene_id)
                              }) %>% Reduce(intersect, .)

blood_early_universe <- lapply(all_deseq2_tp_organ_split_nice[c(1:2)], 
                              FUN = function(x)x %>% pull(ensembl_gene_id)) %>% 
                              Reduce(intersect, .)

# do ORA
ORA_early_blood <- doORA(blood_early_overlap, 
                         blood_early_universe, 
                         org.Mm.eg.db, 
                         out_dir = out_dir,
                         file_prefix = 'blood_early_DEGs_overlap', overwrite = FALSE)

blood_genes <- ORA_early_blood %>% filter(str_detect(Description, 'healing|coagulation|hemostasis')) %>%
  pull(geneID) %>%
  lapply(., function(x) str_split(x, pattern = '\\/') %>% unlist()) %>%
  unlist() %>%
  unique()

early_blood_samples <- all_samples_info_split$blood %>% filter(wpi %in% c('4 wpi', '8 wpi'))

blood_genes_counts_early <- all_FC_counts_list$blood %>% vst(.) %>%
  .[blood_genes, early_blood_samples %>% pull(SampleID)]

# get z score
blood_early_zscores <-(blood_genes_counts_early - rowMeans(blood_genes_counts_early))/apply(blood_genes_counts_early,1, sd, na.rm = TRUE) 

# put gene symbols as rownames
rownames(blood_early_zscores) <- mouse_feature_meta %>%
  filter(ensembl_gene_id %in% rownames(blood_early_zscores)) %>%
  .[rownames(blood_early_zscores), ] %>%
  pull(gene_symbol)
...
col_annotation <- HeatmapAnnotation(Condition = early_blood_samples$Treatment,
                                    col = list(Condition = c('NBH' = '#D7DBDD', 'RML6' = '#E48017')),
                                    annotation_legend_param = list(Condition = list(title = '')),
                                    simple_anno_size = unit(0.3, 'cm'),
                                    annotation_name_gp = gpar(fontsize = 0), 
                                    annotation_label = gpar(fontsize = 20))

# top_col_annnotation <- HeatmapAnnotation(empty = anno_empty(border = FALSE),
#                                         foo = anno_block(gp = gpar(fill = 'white'), labels =c('4 wpi', '8 wpi')))


early_blood_degs_heatmap <- ComplexHeatmap::Heatmap(blood_early_zscores, 
                        col = Rdbu_colors, 
                        show_row_names = TRUE, 
                        row_names_side = 'left', 
                        show_row_dend = FALSE, 
                        ...
                        column_order = 
                        c('PB-11_blood', 'PB-2_blood', 'PB-9_blood', 
                          'PB-7_blood', 'PB-14_blood', 'PB-16_blood', 
                          'PB-27_blood', 'PB-18_blood', 'PB-25_blood', 
                          'PB-31_blood', 'PB-23_blood', 'PB-30_blood'), 
                        column_split = early_blood_samples$wpi, 
...
ggsave(deseq2_dotplot_file, deseq_dotplot, width = 5, height = 8)
